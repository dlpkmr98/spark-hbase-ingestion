 ...
 *
 */
object HBaseSupport extends Serializable with HBaseWrite {
...
trait HBaseWrite {
  ...
  def hbaseContext(implicit sc: SparkContext, conf: Configuration): HBaseContext = new HBaseContext(sc, conf)

  /**
   * @param s
   * @return
   */
  def ct(s: String): Array[Byte] = s
}